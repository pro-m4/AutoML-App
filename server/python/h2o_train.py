...
def run_h2o():
    try:
        ...
        # --- ΚΡΙΣΙΜΗ ΠΡΟΣΘΗΚΗ ---
        # Για Regression, βεβαιωνόμαστε ότι ο στόχος είναι NUMERIC
        if task_type != 'classification':
            df_tmp[target_column] = pd.to_numeric(df_tmp[target_column], errors='coerce')
            df_tmp = df_tmp.dropna(subset=[target_column]) # Διώχνουμε γραμμές χωρίς στόχο

        # Μετατροπή σε H2O Frame
        data = h2o.H2OFrame(df_tmp)
        ...
        if task_type == 'classification':
            ...
            # Regression Metrics Mapping για το Leaderboard
            if user_metric == 'mae':
                sort_metric = 'mae'
            elif user_metric == 'rmse':
                sort_metric = 'rmse'
            elif user_metric == 'mse':
                sort_metric = 'mse' # Το H2O υποστηρίζει MSE απευθείας
            else:
                sort_metric = 'rmse'

        # Δημιουργία του AutoML
        aml = H2OAutoML(max_runtime_secs=time_limit, seed=123, sort_metric=sort_metric)
        
        # Αν ο χρήστης διάλεξε συγκεκριμένα features, τα καθαρίζουμε από κενά
        if features:
            features = [f.strip() for f in features if f.strip() != '']

        aml.train(x=features, y=target_column, training_frame=data)
        ...
        # 5. Scoring με Scikit-Learn (Ο "Διαιτητής")
        preds_h2o = leader.predict(data)
        y_true_df = data[target_column].as_data_frame()
        y_pred_df = preds_h2o.as_data_frame()
        
        # ΠΡΩΤΑ ΟΡΙΖΟΥΜΕ ΤΙΣ ΜΕΤΑΒΛΗΤΕΣ
        y_true_raw = y_true_df.iloc[:, 0]
        y_pred_raw = y_pred_df.iloc[:, 0]
        
        if task_type == 'classification':
            # ΔΙΑΣΦΑΛΙΣΗ ΟΤΙ ΧΡΗΣΙΜΟΠΟΙΟΥΜΕ ΤΙΣ RAW ΜΕΤΑΒΛΗΤΕΣ
            ...
            # --- ΕΛΕΓΧΟΣ ΓΙΑ LOG-SCALE (Το "μυστικό" για το 0.66) ---
            # Αν ο μέσος όρος των πραγματικών είναι π.χ. 100 και των προβλέψεων 4, 
            # τότε το H2O μας έδωσε λογαρίθμους.
            ...
            # ΤΩΡΑ ΜΠΟΡΟΥΜΕ ΝΑ ΚΑΝΟΥΜΕ ΤΑ PRINT ΧΩΡΙΣ ERROR
      ...
      # 6. Save & Zip με Framework-Specific Name (ΛΥΣΗ ΓΙΑ ΤΟ OVERWRITE)
        script_dir = os.path.dirname(os.path.abspath(__file__))
        # Δημιουργούμε έναν μοναδικό φάκελο για το H2O
        ...
        # --- ΕΔΩ ΕΙΝΑΙ Η ΚΡΙΣΙΜΗ ΑΛΛΑΓΗ ---
        # Ονομάζουμε το αρχείο με κατάληξη _h2o για να μην το σβήνει το MLJAR ή το FLAML
        ...
        # Το όνομα που θα στείλουμε πίσω στον worker
        final_zip_name = f"{model_name_unique}.zip"

        # Καθαρισμός προσωρινού φακέλου
        shutil.rmtree(model_dir)

        # 7. Output (Προσθήκη του framework στο JSON)
        print(json.dumps({
            "status": "success",
            "best_model": final_zip_name, # Θα στείλει π.χ. model_job_245_h2o.zip
        ...
        # Κλείσιμο του H2O Cluster για να ελευθερωθεί η RAM
        try:
